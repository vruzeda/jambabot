...
def post_jamba_to_slack(date):
    ...
    for jamba in all_jamba:
        ...
        if dateString in jamba:
            ...
            jamba = re.sub('<b>', '', jamba)
            jamba = re.sub('</b>', '', jamba)
            jamba = re.sub('<font[^>]*>', '', jamba)
            jamba = re.sub('</font>', '', jamba)
            jamba = re.sub('<br>', '\n', jamba)

            if ZUA:
                jamba = re.sub('sexta-feira', ':pizza:-feira', jamba)
                jamba = re.sub('Frango supremo', ':sparkles:FRANGO SUPREMO:sparkles: :heart:', jamba)
                jamba = re.sub('Penne', 'Pênis', jamba)
                jamba = re.sub('à milanesa', 'ali na mesa', jamba)
